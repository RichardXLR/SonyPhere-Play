...
import com.richard.musicplayer.utils.YTPlayerUtils.validateStatus
...
object YTPlayerUtils {
    ...
    suspend fun playerResponseForPlayback(
        ...
        LogManager.i(TAG, "Iniciando busca de dados de reprodução para vídeo: $videoId")
        ...
        LogManager.d(TAG, "[$videoId] Configuração: signatureTimestamp=$signatureTimestamp, isLoggedIn=$isLoggedIn, sessionId=$sessionId")
        ...
        val (webPlayerPot, webStreamingPot) = getWebClientPoTokenOrNull(videoId, sessionId)?.let {
            ...
            LogManager.w(TAG, "[$videoId] Falha ao obter PoToken - continuando sem tokens")
        }
        ...
        for (clientIndex in (-1 until STREAM_FALLBACK_CLIENTS.size)) {
            ...
            LogManager.d(TAG, "[$videoId] Testando cliente: ${client.clientName}, status: ${streamPlayerResponse?.playabilityStatus?.status}")
            ...
            if (streamPlayerResponse?.playabilityStatus?.status == "OK") {
                ...
                if (validateStatus(streamUrl)) {
                    ...
                    LogManager.i(TAG, "[$videoId] ✅ Stream funcionando encontrado com cliente: ${client.clientName}")
                    break
                } else {
                    Log.w(TAG, "[$videoId] [${client.clientName}] got bad http status code")
                    LogManager.w(TAG, "[$videoId] ❌ Cliente ${client.clientName} retornou status HTTP inválido")
                }
            }
        }

        if (streamPlayerResponse == null) {
            LogManager.e(TAG, "[$videoId] 🚨 ERRO CRÍTICO: Resposta do player é nula após tentar todos os clientes")
            throw Exception("Bad stream player response")
        }
        if (streamPlayerResponse.playabilityStatus.status != "OK") {
            LogManager.e(TAG, "[$videoId] 🚨 Status de playabilidade inválido: ${streamPlayerResponse.playabilityStatus.status}, motivo: ${streamPlayerResponse.playabilityStatus.reason}")
        ...
        if (format == null) {
            LogManager.e(TAG, "[$videoId] 🚨 Formato de áudio não encontrado para qualidade: $audioQuality")
            throw Exception("Could not find format")
        }
        if (streamUrl == null) {
            LogManager.e(TAG, "[$videoId] 🚨 URL do stream não encontrada")
